...
const UserDropdown = (props: Props) => {
    ...
    useQuery(
        ...
        () =>
            auth.user?.id
                ? `${apiUrls.apiUrl_IMS_AccountImageDownloadEndpoint}/${auth.user?.id}?` + new Date().getTime()
                : null,
        {
            enabled: !!auth.user?.id, // Enable the query only when 'user.id' is available
            onSuccess: () => {
                setPhoto(`${apiUrls.apiUrl_IMS_AccountImageDownloadEndpoint}/${auth.user?.id}?` + new Date().getTime())
            }
        }
    )
